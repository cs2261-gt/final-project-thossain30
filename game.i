...
void updateCustomer()
{
    ...
    for (int i = 0; i < 6; i++)
    {
            ...
            customers[i].active)
        {
            ...
            if (collisionBitmapBitmap[((player.worldRow + (int)dy) * (1024) + (player.worldCol + (int)dx))] != ((0) | (0) << 5 | (0) << 10) && player.worldRow + (int)dy > 0 && player.worldCol + (int)dx > 0)
            {
                player.worldRow += dy;
                player.worldCol += dx;
            }
...
void drawHeart()
{
    for (int i = 0; i < 3; i++)
    {
        if (hearts[i].active)
        {
            ...
            shadowOAM[i + 60].attr2 = ((hearts[i].curFrame * 2)*32 + (hearts[i].aniState * 2));
        }
        else
        {
